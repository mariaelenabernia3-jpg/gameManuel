...
const campaignData = [
    // Misión 1
    {
        ...
        baseCredits: 100,
        phases: [
            { waves: [{ type: 'standard', pattern: 'horizontal', count: 6 }] },
            { waves: [{ type: 'standard', pattern: 'sine', count: 8 }] },
            { waves: [{ type: 'fast', pattern: 'dive', count: 5 }, { type: 'standard', pattern: 'horizontal', count: 4 }] },
            { waves: [{ type: 'tank', pattern: 'horizontal', count: 2 }, { type: 'standard', pattern: 'sine', count: 6 }] },
            { bossIndex: 0 }
        ]
    },
    // Misión 2
    {
        rankThresholds: { S: 60000, A: 40000, B: 25000 },
        baseCredits: 150,
        phases: [
            { waves: [{ type: 'fast', pattern: 'arc', count: 8 }] },
            { waves: [{ type: 'standard', pattern: 'sine', count: 10 }] },
            { waves: [{ type: 'tank', pattern: 'horizontal', count: 3 }, { type: 'fast', pattern: 'dive', count: 6 }] },
            { waves: [{ type: 'standard', pattern: 'horizontal', count: 8 }, { type: 'fast', pattern: 'sine', count: 8 }] },
            { bossIndex: 1 }
        ]
    },
    // Añade hasta 10 misiones aquí
];

// --- Variables de Estado del Juego ---
let score, isGameOver, difficultyMultiplier, coinMultiplier, activeControl = 'none';
let currentMissionData, currentPhaseIndex;
let stars = [];
...
const sprites = {
    ...
    powerUps: { tripleShot: loadImage('assets/powerup_triple.png'), shield: loadImage('assets/powerup_shield.png'), bomb: loadImage('assets/powerup_bomb.png'), health: loadImage('assets/powerup_health.png'), companion: loadImage('assets/powerup_companion.png') },
    companionDrone: loadImage('assets/drone.png'),
    minion: loadImage('assets/Enemy.png'),
...
let player = {}, boss = {};
const bosses = [ { name: "Guardián", sprite: sprites.bosses[0], width: 140, height: 70, attackPattern: 'burst', phaseTwoAttack: 'stream' }, { name: "Invasor", sprite: sprites.bosses[1], width: 120, height: 80, attackPattern: 'spiral', phaseTwoAttack: 'walls' }, { name: "Depredador", sprite: sprites.bosses[2], width: 150, height: 60, attackPattern: 'homing', phaseTwoAttack: 'shotgun' } ];
const PLAYER_PROGRESS_KEY = 'aceCraftPlayerProgress';

// --- Lógica de Progreso ---
function getSanitizedPlayerProgress() {
    const d = { currency: 0, selectedShip: 'interceptor', unlockedShips: ['interceptor'], shipUpgrades: { 'interceptor': { damage: 0, firerate: 0 }, 'vanguard': { damage: 0, spread: 0 }, 'striker': { mainDamage: 0, sideDamage: 0 } } };
    let p; try { const s = localStorage.getItem(PLAYER_PROGRESS_KEY); p = s ? JSON.parse(s) : d; if (typeof p !== 'object' || p === null) p = d; } catch { p = d; }
    p.currency = typeof p.currency === 'number' ? p.currency : 0; p.unlockedShips = Array.isArray(p.unlockedShips) ? p.unlockedShips : ['interceptor']; p.shipUpgrades = p.shipUpgrades || {};
    for (const sId in shipsData) if (!p.shipUpgrades[sId]) p.shipUpgrades[sId] = d.shipUpgrades[sId];
    if (!p.unlockedShips.includes(p.selectedShip)) p.selectedShip = 'interceptor';
    return p;
}

// --- Inicio y Lógica del Juego ---
function startGame() {
    const settings = JSON.parse(localStorage.getItem(GAME_SETTINGS_KEY));
    if (!settings || settings.mode !== 'campaign' || !campaignData[settings.mission - 1]) {
        alert("Error: No se ha seleccionado una misión válida. Volviendo al menú.");
        window.location.href = 'menu.html';
        return;
    }
    currentMissionData = campaignData[settings.mission - 1];
    difficultyMultiplier = settings.difficulty;
    coinMultiplier = difficultyMultiplier;
    const pData = getSanitizedPlayerProgress();
    const sId = pData.selectedShip;
    const sBase = shipsData[sId];
    const sUp = pData.shipUpgrades[sId];
    sprites.player = sprites.playerShips[sId];
    const finalStats = { ...sBase };
    if (sId === 'interceptor') { finalStats.damage = sBase.baseDamage + (sUp.damage * sBase.upgrades.damage); finalStats.shootInterval = sBase.baseShootInterval + (sUp.firerate * sBase.upgrades.firerate); }
    else if (sId === 'vanguard') { finalStats.damage = sBase.baseDamage + (sUp.damage * sBase.upgrades.damage); finalStats.spreadAngle = 0.2 + (sUp.spread * sBase.upgrades.spread); }
    else if (sId === 'striker') { finalStats.damage = sBase.baseDamage + (sUp.mainDamage * sBase.upgrades.mainDamage); finalStats.sideDamage = 4 + (sUp.sideDamage * sBase.upgrades.sideDamage); }
    finalStats.speed = sBase.baseSpeed;
...
function resetGame(finalStats) {
    ...
    companion = null;
    currentBackground = sprites.backgrounds[0];
    setupPhase();
}

function setupPhase() {
    for(const key in entityArrays) if(key !== 'playerBullets') entityArrays[key].length = 0;
    const phaseData = currentMissionData.phases[currentPhaseIndex];
    if (phaseData.bossIndex !== undefined) {
        const bossIndex = phaseData.bossIndex;
        const bossData = bosses[bossIndex];
        boss = { ...bossData, x: canvas.width / 2 - bossData.width / 2, y: 50, lastShot: 0, spiralAngle: 0, isPhaseTwo: false };
        boss.health = (200 + bossIndex * 150) * difficultyMultiplier;
        boss.maxHealth = boss.health;
        boss.speed = (2 + bossIndex * 0.2) * difficultyMultiplier;
        boss.shootInterval = Math.max(150, (900 - bossIndex * 100) / difficultyMultiplier);
        damageTakenInBoss = 0;
    } else {
        boss = { health: 0 };
        phaseData.waves.forEach(wave => {
            for (let i = 0; i < wave.count; i++) {
                const x = Math.random() * (canvas.width - 40);
                const y = -40 - Math.random() * 200;
                entityArrays.minions.push(createMinion(wave.type, x, y, wave.pattern));
            }
        });
    }
}
...
function update(dt) {
    ...
    handleControls();
    autoShootPlayer();
    if (boss.health > 0) { if (!boss.isPhaseTwo && boss.health <= boss.maxHealth / 2) enterPhaseTwo(); moveBoss(); shootFromBoss(); }
    updateMinionShooting();
    updateEntities();
    if (player.isFiringLaser) updateLaser();

    const isBossPhase = currentMissionData.phases[currentPhaseIndex].bossIndex !== undefined;
    if ((isBossPhase && boss.health <= 0 && boss.health > -1) || (!isBossPhase && entityArrays.minions.length === 0)) {
        if(isBossPhase) boss.health = -1; 
        if (currentPhaseIndex < 4) {
            currentPhaseIndex++;
            setTimeout(setupPhase, 1000); 
        } else {
            missionComplete();
        }
    }
...
function missionComplete() {
    ...
    const { totalCredits, rankBonus } = calculateRewards(rank);
    const pData = getSanitizedPlayerProgress();
    pData.currency += totalCredits;
    localStorage.setItem(PLAYER_PROGRESS_KEY, JSON.stringify(pData));
    document.getElementById('results-score').textContent = Math.floor(score);
    document.getElementById('results-base-credits').textContent = currentMissionData.baseCredits;
    document.getElementById('results-rank-bonus').textContent = `x${rankBonus}`;
    document.getElementById('results-difficulty-bonus').textContent = `x${coinMultiplier}`;
    document.getElementById('results-total-credits').textContent = totalCredits;
    const rankDisplay = document.getElementById('rank-display');
    rankDisplay.textContent = rank;
    rankDisplay.className = `rank-display rank-${rank}`;
    document.getElementById('mission-complete-modal').style.display = 'block';
}

function calculateRank(finalScore) {
    const thresholds = currentMissionData.rankThresholds;
...
function calculateRewards(rank) {
    let rankBonus = 1.0;
    if (rank === 'S') rankBonus = 2.0;
    else if (rank === 'A') rankBonus = 1.5;
    else if (rank === 'B') rankBonus = 1.2;
    const totalCredits = Math.floor(currentMissionData.baseCredits * rankBonus * coinMultiplier);
    return { totalCredits, rankBonus };
}

document.getElementById('return-to-menu-btn').addEventListener('click', () => { window.location.href = 'menu.html'; });
...
// --- RESTO DE FUNCIONES ---
function draw() { ctx.fillStyle = "#000010"; ctx.fillRect(0, 0, canvas.width, canvas.height); if (currentBackground) ctx.drawImage(currentBackground, 0, 0, canvas.width, canvas.height); drawStars(); drawPlayer(); if (companion && companion.active) drawCompanion(); if (boss && boss.health > 0) drawBoss(); drawProjectiles(); drawMinions(); drawExplosions(); drawPowerUps(); if (player.isFiringLaser) drawLaser(); drawUI(); }
...
function checkPlayerDamage(t) { if (!(Date.now() < player.shieldExpiresAt)) { player.health -= t; if(boss.health > 0) damageTakenInBoss += t; if (player.health <= 0 && !isGameOver) gameOver() } }
...
function drawBoss(){if(!boss||isNaN(boss.x)||isNaN(boss.y))return;const pE=boss.isPhaseTwo&&Math.floor(Date.now()/200)%2===0;if(pE)ctx.filter='brightness(1.5)saturate(2)';if(boss.sprite)ctx.drawImage(boss.sprite,boss.x,boss.y,boss.width,boss.height);else{ctx.fillStyle='red';ctx.fillRect(boss.x,boss.y,boss.width,boss.height)}ctx.filter='none';const t=boss.width,e=8,a=boss.health/boss.maxHealth;if(isNaN(a))return;ctx.fillStyle="#333";ctx.fillRect(boss.x,boss.y-14,t,e);ctx.fillStyle="#ff0000";ctx.fillRect(boss.x,boss.y-14,t*a,e)}
function drawProjectiles(){entityArrays.bossProjectiles.forEach(t=>{if(!t||isNaN(t.x)||isNaN(t.y))return;ctx.fillStyle=t.homing&&Date.now()<t.homingDuration?"orange":"#FF00FF";ctx.beginPath();ctx.arc(t.x,t.y,t.radius,0,2*Math.PI);ctx.fill()});ctx.fillStyle="cyan";entityArrays.playerBullets.forEach(t=>{if(!t||isNaN(t.x)||isNaN(t.y))return;ctx.beginPath();ctx.arc(t.x,t.y,t.radius,0,2*Math.PI);ctx.fill()});ctx.fillStyle="pink";entityArrays.minionProjectiles.forEach(t=>{if(!t||isNaN(t.x)||isNaN(t.y))return;ctx.beginPath();ctx.arc(t.x,t.y,t.radius,0,2*Math.PI);ctx.fill()});ctx.fillStyle="#FFA500";entityArrays.companionBullets.forEach(t=>{if(!t||isNaN(t.x)||isNaN(t.y))return;ctx.beginPath();ctx.arc(t.x,t.y,t.radius,0,2*Math.PI);ctx.fill()});}
function drawMinions(){entityArrays.minions.forEach(t=>{if(!t||isNaN(t.x)||isNaN(t.y)||!t.active)return;if(sprites.minion){if(t.isKamikaze)ctx.filter='hue-rotate(180deg)brightness(1.5)';ctx.drawImage(sprites.minion,t.x,t.y,t.size,t.size);ctx.filter='none';}else{ctx.fillStyle=t.isKamikaze?"#FF5733":("tank"===t.type?"#A04040":"#8B0000");ctx.fillRect(t.x,t.y,t.size,t.size);}if(t.type==='tank'&&t.health>0&&t.health<t.maxHealth){const e=t.size,a=t.health/t.maxHealth;if(isNaN(a))return;ctx.fillStyle="#333";ctx.fillRect(t.x,t.y-8,e,4);ctx.fillStyle="red";ctx.fillRect(t.x,t.y-8,e*a,4)}})}
...
function drawPowerUps(){entityArrays.powerUps.forEach(t=>{if(!t||isNaN(t.x)||isNaN(t.y))return;if(sprites.powerUps[t.type])ctx.drawImage(sprites.powerUps[t.type],t.x,t.y,t.size,t.size);else{ctx.fillStyle='yellow';ctx.fillRect(t.x,t.y,t.size,t.size)}})}
function drawCompanion(){if(!companion||isNaN(companion.x))return;if(sprites.companionDrone)ctx.drawImage(sprites.companionDrone,companion.x,companion.y,companion.size,companion.size);else{ctx.fillStyle='#00FFFF';ctx.fillRect(companion.x,companion.y,companion.size,companion.size);}}
