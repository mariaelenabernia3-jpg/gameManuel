...
// Variables para el seguimiento de logros en una partida
...
const shipsData = {
    'interceptor': { name: 'Interceptor', firePattern: 'single', baseDamage: 10, baseSpeed: 5, baseShootInterval: 450, baseHealth: 100, upgrades: { damage: { increment: 2 }, firerate: { increment: -20 } } },
    'vanguard':    { name: 'Vanguard',    firePattern: 'spread', baseDamage: 7, baseSpeed: 4.5, baseShootInterval: 500, baseHealth: 75,  upgrades: { damage: { increment: 2 }, spread: { increment: 0.05 } } },
    'striker':     { name: 'Striker',     firePattern: 'side',   baseDamage: 12, baseSpeed: 4,   baseShootInterval: 480, baseHealth: 100, upgrades: { mainDamage: { increment: 3 }, sideDamage: { increment: 2 } } }
};
let player = {}; let boss = {}; let minionsDestroyedInLevel = 0; let damageTakenInLevel = 0;
let currentBackground = null;

const bosses = [
    { name: "Guardián", sprite: sprites.bosses[0], width: 140, height: 70, attackPattern: 'burst', phaseTwoAttack: 'stream', minionType: 'standard' },
    { name: "Invasor", sprite: sprites.bosses[1], width: 120, height: 80, attackPattern: 'spiral', phaseTwoAttack: 'walls', minionType: 'fast' },
    { name: "Depredador", sprite: sprites.bosses[2], width: 150, height: 60, attackPattern: 'homing', phaseTwoAttack: 'shotgun', minionType: 'tank' }
];

const achievements = {
    'level5': { title: "Superviviente Nato", unlocked: false, reward: 50 },
    'combo50': { title: "Maestro del Combo", unlocked: false, reward: 50 },
    'noHitBoss': { title: "Intocable", unlocked: false, reward: 100 },
    'minions100': { title: "Aniquilador", unlocked: false, reward: 75 },
    'allPowerups': { title: "Coleccionista", unlocked: false, progress: new Set(), reward: 75 },
    'bossHunter': { title: "Cazador de Gigantes", unlocked: false, reward: 150 },
    'laserMaster': { title: "Poder Desatado", unlocked: false, reward: 50 },
    'closeCall': { title: "Al Filo de la Muerte", unlocked: false, reward: 100 },
    'creditHoarder': { title: "Botín de Guerra", unlocked: false, reward: 100 }
};
...
function showAchievementToast(title, reward) {
    ui.achievementToast.textContent = `¡Logro: ${title}! (+${reward} C)`;
    ui.achievementToast.style.top = '20px';
    setTimeout(() => { ui.achievementToast.style.top = '-100px'; }, 4000);
}
...
function unlockAchievement(id) {
    if (achievements[id] && !achievements[id].unlocked) {
        ...
        showAchievementToast(achievements[id].title, reward);

        const savedProgress = localStorage.getItem(PLAYER_PROGRESS_KEY);
        let playerProgress;
        try { playerProgress = savedProgress ? JSON.parse(savedProgress) : null;
        } catch (e) { playerProgress = null; }
        ...
        if (typeof playerProgress.currency !== 'number' || isNaN(playerProgress.currency)) { playerProgress.currency = 0; }
        
        playerProgress.currency += reward;
...
function startGame(diffMult) {
    ...
    if (diffMult === 0.75) { coinMultiplier = 0.4;
    } else if (diffMult === 1.5) { coinMultiplier = 1.5;
    } else { coinMultiplier = 0.8; }
    ...
    } else if (selectedShipId === 'vanguard') { finalStats.damage = shipBaseStats.baseDamage + (shipUpgrades.damage * shipBaseStats.upgrades.damage.increment); finalStats.spreadAngle = 0.2 + (shipUpgrades.spread * shipBaseStats.upgrades.spread.increment); finalStats.shootInterval = shipBaseStats.baseShootInterval;
    } else if (selectedShipId === 'striker') { finalStats.damage = shipBaseStats.baseDamage + (shipUpgrades.mainDamage * shipBaseStats.upgrades.mainDamage.increment); finalStats.sideDamage = 4 + (shipUpgrades.sideDamage * shipBaseStats.upgrades.sideDamage.increment); finalStats.shootInterval = shipBaseStats.baseShootInterval; }
...
function update(dt) {
    ...
    if (boss.health > 0) {
        if (boss.health <= boss.maxHealth / 2 && !boss.isPhaseTwo) { enterPhaseTwo(); }
        moveBoss(); shootFromBoss();
    }
...
function draw() { if (currentBackground && currentBackground.loaded) { ctx.drawImage(currentBackground, 0, 0, canvas.width, canvas.height); } else { ctx.fillStyle = "#000010"; ctx.fillRect(0, 0, canvas.width, canvas.height); } drawStars(); drawPlayer(); if (boss && boss.health > 0) drawBoss(); drawProjectiles(); drawMinions(); drawExplosions(); drawPowerUps(); if (player.isFiringLaser) drawLaser(); drawUI(); }
...
function saveAndShowScores() { isGameOver = true; const savedProgress = localStorage.getItem(PLAYER_PROGRESS_KEY); let playerProgress; try { playerProgress = savedProgress ? JSON.parse(savedProgress) : null; } catch (error) { console.error("No se pudo parsear el progreso del jugador. Se reiniciará.", error); playerProgress = null; } if (!playerProgress || typeof playerProgress !== 'object') { playerProgress = { currency: 0, selectedShip: 'interceptor', unlockedShips: ['interceptor'], shipUpgrades: { 'interceptor': { damage: 0, firerate: 0 }, 'vanguard': { damage: 0, spread: 0 }, 'striker': { mainDamage: 0, sideDamage: 0 } } }; } if (typeof playerProgress.currency !== 'number' || isNaN(playerProgress.currency)) { playerProgress.currency = 0; } playerProgress.currency += Math.floor(coinsEarnedThisGame); localStorage.setItem(PLAYER_PROGRESS_KEY, JSON.stringify(playerProgress)); const name = prompt("Fin de la Misión. Registra tu nombre de piloto:", "PILOTO"); try { const highScores = JSON.parse(localStorage.getItem(HIGH_SCORES_KEY)) || []; const newScore = { name: name || "PILOTO", score: score }; highScores.push(newScore); highScores.sort((a, b) => b.score - a.score); highScores.splice(5); localStorage.setItem(HIGH_SCORES_KEY, JSON.stringify(highScores)); ui.highscoreList.innerHTML = ''; highScores.forEach(scoreEntry => { const li = document.createElement('li'); li.innerHTML = `<span class="name">${scoreEntry.name}</span> <span class="score">${scoreEntry.score}</span>`; ui.highscoreList.appendChild(li); }); } catch (e) { console.error("Error al procesar las puntuaciones", e); ui.highscoreList.innerHTML = '<li>Error al cargar puntuaciones</li>'; } ui.highscoreTable.style.display = 'block'; }
